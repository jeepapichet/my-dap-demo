...
platform_image() {
## == modify to use version tag from image detail
  if [ $PLATFORM = "openshift" ]; then
#    echo "$DOCKER_REGISTRY_PATH/$CONJUR_NAMESPACE_NAME/$1:$CONJUR_NAMESPACE_NAME"
    echo "$DOCKER_REGISTRY_PATH/$CONJUR_NAMESPACE_NAME/$1"
...
platform_image_internal() {
## == Return path using internal address
  if [ $PLATFORM = "openshift" ]; then
    echo "$DOCKER_INTERNAL_REGISTRY_PATH/$CONJUR_NAMESPACE_NAME/$1"
  elif ! is_minienv; then
    echo "$DOCKER_INTERNAL_REGISTRY_PATH/$1"
  else
    echo "$1"
  fi
}
